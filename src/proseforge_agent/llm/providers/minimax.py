...
Speaks MiniMax's OpenAI-compatible Chat Completions API (Bearer auth,
``/chat/completions``, with a ``reasoning_content`` field on thinking models and
multimodal metadata) behind the normalized provider contract: workflows receive
a :class:`ProviderResult` and never see a raw MiniMax payload. The visible answer
is normalized into ``result.text``; thinking extras and multimodal metadata stay
in ``result.raw`` (the trace is read via :func:`reasoning_text`). The API key is
...
MINIMAX_ALIASES: tuple[str, ...] = ("minimax", "minimax_chat")
...
SOURCE_NOTE = "MiniMax official API reference (platform.minimaxi.com/document)"
...
class MiniMaxProvider:
    """Call MiniMax's OpenAI-compatible Chat Completions endpoint."""
...
# -- thinking-extras accessor ------------------------------------------------


def reasoning_text(result: ProviderResult) -> str:
    """Return MiniMax's thinking trace (``reasoning_content``) from a result.

    The reasoning trace is kept out of ``result.text`` and lives in
    ``result.raw``; this reads it safely, returning ``""`` if absent.
    """
    try:
...
def build_provider(
    ...
    """Build a :class:`MiniMaxProvider` from a profile (no key required offline)."""
    api_key = os.environ.get(profile.api_key_env or "MINIMAX_API_KEY", "")
...
def real_smoke(profile: ProviderProfile) -> dict:
    ...
    env = profile.api_key_env or "MINIMAX_API_KEY"
