# Task 142: Job Status Center / 后台任务状态中心

## Goal

给后台任务提供统一状态查询、日志和通知联动。
...
```bash id="08okvt"
pf-agent jobs list
pf-agent jobs status <job_id>
pf-agent jobs logs <job_id>
pf-agent jobs cancel <job_id>
```

## Files

- Create or modify implementation files under `src/proseforge_agent/notifications/` as needed for this card.
- Add focused tests in `tests/notifications/test_job_status_center.py`.
- Add fixtures under `tests/notifications/fixtures/job-status-center/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/notifications/test_job_status_center.py::test_job_status_center_contract`**

```python
def test_job_status_center_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 142 production code is not implemented yet.
    raise AssertionError("Task 142 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/notifications/test_job_status_center.py::test_job_status_center_contract -q
```

Expected: FAIL because Task 142 production behavior is missing.
...
Expected: PASS with the new Task 142 behavior covered.
...
```powershell
git add src/proseforge_agent/notifications/ tests/notifications/test_job_status_center.py
git commit -m "feat: add job status center"
```
...
export / backup / rag ingest 这类长任务能在 jobs center 查询状态。
...
```powershell
python -m pytest tests/notifications/test_job_status_center.py -q
python -m pytest -q
```
...
Commit only Task 142 implementation files, tests, and required fixtures:
