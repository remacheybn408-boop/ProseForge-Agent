...
from proseforge_agent.llm.providers.doubao import (
    build_provider,
    real_smoke,
    region_of,
    shape_certification,
...
CONFIG = "configs/providers/doubao.yaml"
SUCCESS = (FIXTURES / "doubao_success.json").read_text(encoding="utf-8")
ERROR = (FIXTURES / "doubao_error.json").read_text(encoding="utf-8")
STREAM = (FIXTURES / "doubao_stream.txt").read_text(encoding="utf-8").splitlines()


def _profile(name="doubao_main"):
...
def test_doubao_request_shape_uses_profile_config(fake_http, monkeypatch):
    ...
    profile = load_provider_profiles(CONFIG)["doubao_main"]
...
def test_doubao_parses_success_response_into_provider_result(fake_http):
    result = build_provider(_profile(), http=fake_http).generate(_request())
    assert result.text == "A single quiet line."
    assert result.usage.total_tokens > 0
    assert result.raw["id"] == "chatcmpl_doubao_0001"
    assert result.raw["choices"][0]["finish_reason"] == "stop"


def test_doubao_endpoint_id_used_as_model():
    http = FakeHttpTransport(responses=[HttpResponse(status_code=200, text=SUCCESS)])
    build_provider(_profile("doubao_endpoint"), http=http).generate(_request())
    assert http.requests[0].json["model"].startswith("ep-")


def test_doubao_region_parsed_from_base_url():
    assert region_of(_profile().base_url) == "cn-beijing"
    assert region_of("https://example.com/v1") == ""


def test_doubao_parses_stream_events_into_normalized_chunks():
    http = FakeHttpTransport(stream_lines=STREAM)
    chunks = list(build_provider(_profile(), http=http).generate_stream(_request()))
    assert "".join(c.text for c in chunks) == "Hello from Doubao"
    assert chunks[-1].done is True


def test_doubao_maps_auth_rate_limit_timeout_and_invalid_response_errors():
...
def test_doubao_does_not_leak_key_in_errors(monkeypatch):
    monkeypatch.setenv("ARK_API_KEY", "secret-XYZ")
...
def test_doubao_missing_ark_api_key_skips_real_smoke(monkeypatch):
...
def test_doubao_shape_certification_report_records_source_and_checked_date(monkeypatch):
    monkeypatch.delenv("ARK_API_KEY", raising=False)
...
def test_registry_resolves_doubao_aliases(fake_http):
