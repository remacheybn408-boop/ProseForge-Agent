# Task 103: Project Health Doctor / 小说项目健康检查

## Goal

提供专门针对小说项目的 doctor，不是环境 doctor。
...
```bash
pf-agent project doctor --slug demo_novel
pf-agent project doctor --slug demo_novel --fix
```

## Files

- Create or modify implementation files under `src/proseforge_agent/novel/` as needed for this card.
- Add focused tests in `tests/novel/test_project_health_doctor.py`.
- Add fixtures under `tests/novel/fixtures/project-health-doctor/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/novel/test_project_health_doctor.py::test_project_health_doctor_contract`**

```python
def test_project_health_doctor_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 103 production code is not implemented yet.
    raise AssertionError("Task 103 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/novel/test_project_health_doctor.py::test_project_health_doctor_contract -q
```

Expected: FAIL because Task 103 production behavior is missing.
...
Expected: PASS with the new Task 103 behavior covered.
...
```powershell
git add src/proseforge_agent/novel/ tests/novel/test_project_health_doctor.py
git commit -m "feat: add project health doctor"
```
...
```powershell
python -m pytest tests/novel/test_project_health_doctor.py -q
python -m pytest -q
```
...
Commit only Task 103 implementation files, tests, and required fixtures:
