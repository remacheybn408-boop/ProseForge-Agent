# Task 144: Plugin Discovery / 插件发现
...
```bash id="etbsy2"
pf-agent plugin list
pf-agent plugin discover
pf-agent plugin info <plugin_id>
```

## Files

- Create or modify implementation files under `src/proseforge_agent/plugins/` as needed for this card.
- Add focused tests in `tests/plugins/test_plugin_discovery.py`.
- Add fixtures under `tests/plugins/fixtures/plugin-discovery/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/plugins/test_plugin_discovery.py::test_plugin_discovery_contract`**

```python
def test_plugin_discovery_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 144 production code is not implemented yet.
    raise AssertionError("Task 144 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/plugins/test_plugin_discovery.py::test_plugin_discovery_contract -q
```

Expected: FAIL because Task 144 production behavior is missing.
...
Expected: PASS with the new Task 144 behavior covered.
...
```powershell
git add src/proseforge_agent/plugins/ tests/plugins/test_plugin_discovery.py
...
插件放入 plugin directory 后，`pf-agent plugin list` 能显示。
...
```powershell
python -m pytest tests/plugins/test_plugin_discovery.py -q
python -m pytest -q
```
...
Commit only Task 144 implementation files, tests, and required fixtures:
