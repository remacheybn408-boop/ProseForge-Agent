# Task 91: Plot Thread Manager / 情节线管理器

## Goal

管理主线、支线、暗线、感情线、反派线，避免长篇后期线索丢失。
...
```bash
pf-agent plot-thread add --slug demo_novel
pf-agent plot-thread list --slug demo_novel
pf-agent plot-thread stale --slug demo_novel
```

## Files

- Create or modify implementation files under `src/proseforge_agent/novel/` as needed for this card.
- Add focused tests in `tests/novel/test_plot_thread_manager.py`.
- Add fixtures under `tests/novel/fixtures/plot-thread-manager/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/novel/test_plot_thread_manager.py::test_plot_thread_manager_contract`**

```python
def test_plot_thread_manager_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 91 production code is not implemented yet.
    raise AssertionError("Task 91 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/novel/test_plot_thread_manager.py::test_plot_thread_manager_contract -q
```

Expected: FAIL because Task 91 production behavior is missing.
...
Expected: PASS with the new Task 91 behavior covered.
...
```powershell
git add src/proseforge_agent/novel/ tests/novel/test_plot_thread_manager.py
git commit -m "feat: add plot thread manager"
```
...
```powershell
python -m pytest tests/novel/test_plot_thread_manager.py -q
python -m pytest -q
```
...
Commit only Task 91 implementation files, tests, and required fixtures:
