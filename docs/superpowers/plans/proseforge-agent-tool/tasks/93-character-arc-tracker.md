# Task 93: Character Arc Tracker / 人物弧光追踪

## Goal

追踪每个角色的目标、变化、关系、关键事件和心理状态变化。
...
```bash
pf-agent character-arc init --slug demo_novel --character char_001
pf-agent character-arc update --slug demo_novel --character char_001
pf-agent character-arc report --slug demo_novel
```

## Files

- Create or modify implementation files under `src/proseforge_agent/novel/` as needed for this card.
- Add focused tests in `tests/novel/test_character_arc_tracker.py`.
- Add fixtures under `tests/novel/fixtures/character-arc-tracker/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/novel/test_character_arc_tracker.py::test_character_arc_tracker_contract`**

```python
def test_character_arc_tracker_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 93 production code is not implemented yet.
    raise AssertionError("Task 93 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/novel/test_character_arc_tracker.py::test_character_arc_tracker_contract -q
```

Expected: FAIL because Task 93 production behavior is missing.
...
Expected: PASS with the new Task 93 behavior covered.
...
```powershell
git add src/proseforge_agent/novel/ tests/novel/test_character_arc_tracker.py
git commit -m "feat: add character arc tracker"
```
...
```powershell
python -m pytest tests/novel/test_character_arc_tracker.py -q
python -m pytest -q
```
...
Commit only Task 93 implementation files, tests, and required fixtures:
