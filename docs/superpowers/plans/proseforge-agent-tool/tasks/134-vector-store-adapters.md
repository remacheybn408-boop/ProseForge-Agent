# Task 134: Vector Store Adapters / 向量库适配器
...
- Add focused tests in `tests/retrieval/test_vector_store_adapters.py`.
- Add fixtures under `tests/retrieval/fixtures/vector-store-adapters/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/retrieval/test_vector_store_adapters.py::test_vector_store_adapters_contract`**

```python
def test_vector_store_adapters_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 134 production code is not implemented yet.
    raise AssertionError("Task 134 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/retrieval/test_vector_store_adapters.py::test_vector_store_adapters_contract -q
```

Expected: FAIL because Task 134 production behavior is missing.
...
Expected: PASS with the new Task 134 behavior covered.
...
```powershell
git add src/proseforge_agent/retrieval/ tests/retrieval/test_vector_store_adapters.py
...
fake embedding + local vector store 可以完成最小语义搜索。
...
```powershell
python -m pytest tests/retrieval/test_vector_store_adapters.py -q
python -m pytest -q
```
...
Commit only Task 134 implementation files, tests, and required fixtures:
