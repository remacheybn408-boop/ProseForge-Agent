# Task 101: Reader Experience Review / 读者体验审稿

## Goal

提供更接近小说编辑的读者体验分析，而不是普通文本 review。
...
```bash
pf-agent reader-review --slug demo_novel --chapter ch_001
pf-agent reader-review --slug demo_novel --volume vol_001
```

## Files

- Create or modify implementation files under `src/proseforge_agent/novel/` as needed for this card.
- Add focused tests in `tests/novel/test_reader_experience_review.py`.
- Add fixtures under `tests/novel/fixtures/reader-experience-review/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/novel/test_reader_experience_review.py::test_reader_experience_review_contract`**

```python
def test_reader_experience_review_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 101 production code is not implemented yet.
    raise AssertionError("Task 101 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/novel/test_reader_experience_review.py::test_reader_experience_review_contract -q
```

Expected: FAIL because Task 101 production behavior is missing.
...
Expected: PASS with the new Task 101 behavior covered.
...
```powershell
git add src/proseforge_agent/novel/ tests/novel/test_reader_experience_review.py
git commit -m "feat: add reader experience review"
```
...
输出结构化 reader report，并给出可执行修改建议。
...
```powershell
python -m pytest tests/novel/test_reader_experience_review.py -q
python -m pytest -q
```
...
Commit only Task 101 implementation files, tests, and required fixtures:
