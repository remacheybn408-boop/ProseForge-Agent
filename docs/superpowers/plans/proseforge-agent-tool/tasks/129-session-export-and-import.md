# Task 129: Session Export / Import / 会话导入导出

## Goal

支持会话导出为 JSON / Markdown，并可重新导入。
...
```bash id="tw266k"
pf-agent session export <id> --format markdown
pf-agent session export <id> --format json
pf-agent session import ./session.json
```
...
* secrets 脱敏
* tool args 可配置是否导出
* evidence pack 可配置是否导出

## Files

- Create or modify implementation files under `src/proseforge_agent/chat/` as needed for this card.
- Add focused tests in `tests/chat/test_session_export_and_import.py`.
- Add fixtures under `tests/chat/fixtures/session-export-and-import/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/chat/test_session_export_and_import.py::test_session_export_and_import_contract`**

```python
def test_session_export_and_import_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 129 production code is not implemented yet.
    raise AssertionError("Task 129 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/chat/test_session_export_and_import.py::test_session_export_and_import_contract -q
```

Expected: FAIL because Task 129 production behavior is missing.
...
Expected: PASS with the new Task 129 behavior covered.
...
```powershell
git add src/proseforge_agent/chat/ tests/chat/test_session_export_and_import.py
git commit -m "feat: add session export and import"
```
...
导出 markdown 后能阅读完整会话过程；导出 json 后能导入恢复。
...
```powershell
python -m pytest tests/chat/test_session_export_and_import.py -q
python -m pytest -q
```
...
Commit only Task 129 implementation files, tests, and required fixtures:
