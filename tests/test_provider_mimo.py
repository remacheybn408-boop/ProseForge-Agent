...
from proseforge_agent.llm.profiles import ProviderProfile, load_provider_profiles
from proseforge_agent.llm.providers.mimo import (
    build_provider,
    real_smoke,
    shape_certification,
)
...
CONFIG = "configs/providers/mimo.yaml"
SUCCESS = (FIXTURES / "mimo_success.json").read_text(encoding="utf-8")
ERROR = (FIXTURES / "mimo_error.json").read_text(encoding="utf-8")
STREAM = (FIXTURES / "mimo_stream.txt").read_text(encoding="utf-8").splitlines()


def _profile(name="mimo_main"):
...
def test_mimo_request_shape_uses_profile_config(fake_http, monkeypatch):
    ...
    profile = load_provider_profiles(CONFIG)["mimo_main"]
...
def test_mimo_parses_success_response_into_provider_result(fake_http):
    result = build_provider(_profile(), http=fake_http).generate(_request())
    assert result.text == "A single quiet line."
    assert result.usage.total_tokens > 0
    assert result.raw["id"] == "chatcmpl_mimo_0001"
    assert result.raw["choices"][0]["finish_reason"] == "stop"


def test_mimo_requires_configured_base_url():
    profile = ProviderProfile(
        name="mimo_no_base",
        family="mimo",
        protocol="mimo_openai",
        model="mimo-7b",
        base_url=None,
        api_key_env="MIMO_API_KEY",
    )
    with pytest.raises(ConfigurationError):
        build_provider(profile, http=FakeHttpTransport())


def test_mimo_parses_stream_events_into_normalized_chunks():
    http = FakeHttpTransport(stream_lines=STREAM)
    chunks = list(build_provider(_profile(), http=http).generate_stream(_request()))
    assert "".join(c.text for c in chunks) == "Hello from MiMo"
    assert chunks[-1].done is True


def test_mimo_maps_auth_rate_limit_timeout_and_invalid_response_errors():
...
def test_mimo_does_not_leak_key_in_errors(monkeypatch):
    monkeypatch.setenv("MIMO_API_KEY", "secret-XYZ")
...
def test_mimo_missing_mimo_api_key_skips_real_smoke(monkeypatch):
    monkeypatch.delenv("MIMO_API_KEY", raising=False)
    report = real_smoke(_profile())
    assert report["skipped"] is True
    assert "MIMO_API_KEY" in report["reason"]


def test_mimo_shape_certification_report_records_source_and_checked_date(monkeypatch):
    monkeypatch.delenv("MIMO_API_KEY", raising=False)
...
def test_registry_resolves_mimo_aliases(fake_http):
