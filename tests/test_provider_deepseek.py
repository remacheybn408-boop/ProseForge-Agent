...
from proseforge_agent.llm.providers.deepseek import (
    build_provider,
    real_smoke,
    reasoning_text,
    shape_certification,
...
CONFIG = "configs/providers/deepseek.yaml"
SUCCESS = (FIXTURES / "deepseek_success.json").read_text(encoding="utf-8")
ERROR = (FIXTURES / "deepseek_error.json").read_text(encoding="utf-8")
STREAM = (FIXTURES / "deepseek_stream.txt").read_text(encoding="utf-8").splitlines()


def _profile(name="deepseek_main"):
...
def test_deepseek_request_shape_uses_profile_config(fake_http, monkeypatch):
    ...
    profile = load_provider_profiles(CONFIG)["deepseek_main"]
...
def test_deepseek_parses_success_response_into_provider_result(fake_http):
    result = build_provider(_profile(), http=fake_http).generate(_request())
    assert result.text == "A single quiet line."
    assert result.usage.total_tokens > 0
    assert result.raw["id"] == "chatcmpl_deepseek_0001"
    assert result.raw["choices"][0]["finish_reason"] == "stop"


def test_deepseek_parses_reasoning_metadata(fake_http):
    result = build_provider(_profile(), http=fake_http).generate(_request())
    assert reasoning_text(result) == "Think: keep it short and calm."
    assert "Think:" not in result.text


def test_deepseek_parses_stream_events_into_normalized_chunks():
    http = FakeHttpTransport(stream_lines=STREAM)
    chunks = list(build_provider(_profile(), http=http).generate_stream(_request()))
    assert "".join(c.text for c in chunks) == "Hello from DeepSeek"
    assert chunks[-1].done is True


def test_deepseek_maps_auth_rate_limit_timeout_and_invalid_response_errors():
...
def test_deepseek_does_not_leak_key_in_errors(monkeypatch):
    monkeypatch.setenv("DEEPSEEK_API_KEY", "secret-XYZ")
...
def test_deepseek_missing_deepseek_api_key_skips_real_smoke(monkeypatch):
    monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)
    report = real_smoke(_profile())
    assert report["skipped"] is True
    assert "DEEPSEEK_API_KEY" in report["reason"]


def test_deepseek_shape_certification_report_records_source_and_checked_date(monkeypatch):
    monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)
...
def test_registry_resolves_deepseek_aliases(fake_http):
