# Task 124: Graceful Degradation / 功能分级降级

## Goal

当模型、网络、工具、MCP、memory 部分不可用时，系统仍能以低能力模式运行。

## Architecture Notes

This card belongs to the **Resilience And Offline Operation** track. Keep the implementation focused in the existing ProseForge Agent architecture and reuse established CLI, report, config, permission, and persistence patterns where they already exist.

### Source Core Requirements

每个功能声明最低运行等级：

```yaml id="xl5sbk"
features:
  chat:
    min_level: 3
  export:
    min_level: 4
  rewrite:
    min_level: 1
  mcp_tools:
    min_level: 0
```

## Files

- Create or modify implementation files under `src/proseforge_agent/agent/` as needed for this card.
- Add focused tests in `tests/agent/test_graceful_degradation.py`.
- Add fixtures under `tests/agent/fixtures/graceful-degradation/` only when deterministic sample data is needed.
- Update CLI/report wiring only if the source card explicitly requires a command or user-visible surface.

## Interfaces / Contracts

### Source 降级等级

```text id="bt4lh8"
Level 0: full online agent
Level 1: provider available, tools degraded
Level 2: provider available, memory read-only
Level 3: fake/local provider only
Level 4: local operations only
```

## TDD Steps

- [ ] **Step 1: Write failing unit test `tests/agent/test_graceful_degradation.py::test_graceful_degradation_contract`**

```python
def test_graceful_degradation_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 124 production code is not implemented yet.
    raise AssertionError("Task 124 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/agent/test_graceful_degradation.py::test_graceful_degradation_contract -q
```

Expected: FAIL because Task 124 production behavior is missing.

- [ ] **Step 3: Implement the minimum production behavior**

Implement the smallest cohesive change under `src/proseforge_agent/agent/` that satisfies the source requirements and keeps unrelated subsystems unchanged.

- [ ] **Step 4: Run the targeted test and confirm pass**

```powershell
python -m pytest tests/agent/test_graceful_degradation.py::test_graceful_degradation_contract -q
```

Expected: PASS with the new Task 124 behavior covered.

- [ ] **Step 5: Add companion tests for the source DoD and failure modes**

Cover all command/API/config behaviors named in this card, including non-destructive behavior, no-secret-leak behavior, permission checks, degraded/offline behavior, or deterministic fake-provider behavior when relevant.

- [ ] **Step 6: Run subsystem verification**

```powershell
python -m pytest tests/agent/ -q
```

Expected: subsystem tests pass.

- [ ] **Step 7: Run full repository verification**

```powershell
python -m pytest -q
```

Expected: full suite passes.

- [ ] **Step 8: Record commit boundary**

```powershell
git add src/proseforge_agent/agent/ tests/agent/test_graceful_degradation.py
git commit -m "feat: add graceful degradation"
```

## Verification

Source DoD:

网络不可用时，`pf-agent export` 仍可执行，`pf-agent rewrite` 给出降级说明。

---

Before closing this card, run:

```powershell
python -m pytest tests/agent/test_graceful_degradation.py -q
python -m pytest -q
```

## Acceptance

- The behavior described in the source goal is implemented and covered by tests.
- Any command/API/config surface named in the source card works through the existing project conventions.
- The implementation is deterministic in tests and does not require real network/provider credentials unless the card explicitly says so.
- Existing user data, project artifacts, secrets, and config are preserved unless this card explicitly requires a safe migration or backup.

## Commit Boundary

Commit only Task 124 implementation files, tests, and required fixtures:

```powershell
git add src/proseforge_agent/agent/ tests/agent/test_graceful_degradation.py
git commit -m "feat: add graceful degradation"
```

Do not bundle adjacent task cards into this commit.
