...
Speaks DeepSeek's OpenAI-compatible Chat Completions API (Bearer auth,
``/chat/completions``, with a ``reasoning_content`` field on reasoning models)
behind the normalized provider contract: workflows receive a
:class:`ProviderResult` and never see a raw DeepSeek payload. The visible answer
is normalized into ``result.text``; chain-of-thought stays in ``result.raw`` and
is read via :func:`reasoning_text`. The API key is read from the environment
...
DEEPSEEK_ALIASES: tuple[str, ...] = ("deepseek", "deepseek_chat")
...
SOURCE_NOTE = "DeepSeek official API reference (api-docs.deepseek.com)"
...
class DeepSeekProvider:
    ...
    def _extract_delta(obj: dict) -> str:
        ...
        if choices:
            # Only the visible answer counts as streamed text; the reasoning
            # delta (`reasoning_content`) is intentionally not surfaced here.
...
# -- reasoning metadata accessor ---------------------------------------------


def reasoning_text(result: ProviderResult) -> str:
    """Return DeepSeek's chain-of-thought (``reasoning_content``) from a result.
...
def build_provider(
    ...
    """Build a :class:`DeepSeekProvider` from a profile (no key required offline)."""
    api_key = os.environ.get(profile.api_key_env or "DEEPSEEK_API_KEY", "")
...
def real_smoke(profile: ProviderProfile) -> dict:
    ...
    env = profile.api_key_env or "DEEPSEEK_API_KEY"
