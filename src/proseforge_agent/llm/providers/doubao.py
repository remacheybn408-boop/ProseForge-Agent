"""Volcengine Doubao (Ark) provider profile.

Speaks Volcengine Ark's OpenAI-compatible Chat Completions surface (Bearer auth,
``/chat/completions``) behind the normalized provider contract: workflows
receive a :class:`ProviderResult` and never see a raw Doubao payload. Ark's
``model`` field accepts either a model name or an inference endpoint id
(``ep-...``); the region is carried by the base URL host. The API key is read
from the environment named by the profile, is redacted from request reprs (the
``Authorization`` header), and never appears in raised errors.
"""
...
DOUBAO_ALIASES: tuple[str, ...] = ("doubao", "volcengine", "ark")
...
SOURCE_NOTE = "Volcengine Ark (Doubao) official API reference (volcengine.com/docs/82379)"
_DEFAULT_BASE_URL = "https://ark.cn-beijing.volces.com/api/v3"
_REGION_RE = re.compile(r"ark\.([a-z0-9-]+)\.volces\.com", re.IGNORECASE)
...
def region_of(base_url: str | None) -> str:
    """Parse the Ark region from a base URL host (e.g. ``cn-beijing``).

    Returns ``""`` when the host does not match the Ark pattern.
    """
    if not base_url:
        return ""
    match = _REGION_RE.search(base_url)
    return match.group(1) if match else ""


class DoubaoProvider:
    """Call Volcengine Ark's OpenAI-compatible Chat Completions endpoint."""
    ...
    def __init__(
        ...
        *,
        ...
        self.region = region_of(base_url)
    ...
    def _body(self, request: ProviderRequest, *, stream: bool) -> dict:
        # `model` carries either a model name or an inference endpoint id (ep-...).
        body: dict = {
            "model": self.model,
...
def build_provider(profile: ProviderProfile, *, http: HttpTransport | None = None) -> DoubaoProvider:
    """Build a :class:`DoubaoProvider` from a profile (no key required offline)."""
    api_key = os.environ.get(profile.api_key_env or "ARK_API_KEY", "")
...
def real_smoke(profile: ProviderProfile) -> dict:
    ...
    env = profile.api_key_env or "ARK_API_KEY"
