# Task 123: Provider Fallback / Provider 降级

## Goal

当主 provider 不可用时，自动降级到备用 provider。
...
- Add focused tests in `tests/agent/test_provider_fallback.py`.
- Add fixtures under `tests/agent/fixtures/provider-fallback/` only when deterministic sample data is needed.
...
```bash id="4e0078"
pf-agent provider fallback-status
pf-agent provider test-chain
```

## TDD Steps

- [ ] **Step 1: Write failing unit test `tests/agent/test_provider_fallback.py::test_provider_fallback_contract`**

```python
def test_provider_fallback_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 123 production code is not implemented yet.
    raise AssertionError("Task 123 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/agent/test_provider_fallback.py::test_provider_fallback_contract -q
```

Expected: FAIL because Task 123 production behavior is missing.
...
Expected: PASS with the new Task 123 behavior covered.
...
```powershell
git add src/proseforge_agent/agent/ tests/agent/test_provider_fallback.py
git commit -m "feat: add provider fallback"
```
...
DeepSeek 模拟超时后，系统自动尝试 Qwen，再失败则尝试 fake。
...
```powershell
python -m pytest tests/agent/test_provider_fallback.py -q
python -m pytest -q
```
...
Commit only Task 123 implementation files, tests, and required fixtures:
