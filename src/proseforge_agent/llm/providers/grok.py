...
Speaks xAI's OpenAI-style API surface (Bearer auth, ``/chat/completions``,
with an ``output_text`` field on some responses) behind the normalized provider
contract: workflows receive a :class:`ProviderResult` and never see a raw xAI
payload. The API key is read from the environment named by the profile, is
redacted from request reprs (the ``Authorization`` header), and never appears in
raised errors.
"""
...
GROK_ALIASES: tuple[str, ...] = ("xai", "grok")
...
SOURCE_NOTE = "xAI official API reference (docs.x.ai)"
...
class GrokProvider:
    """Call xAI's OpenAI-style Chat Completions endpoint."""
    ...
    def _extract_text(payload: dict) -> str:
        # Prefer a responses-style `output_text`, then `output[]`, then fall back
        # to Chat Completions `choices`.
...
def build_provider(profile: ProviderProfile, *, http: HttpTransport | None = None) -> GrokProvider:
    """Build a :class:`GrokProvider` from a profile (no key required offline)."""
    api_key = os.environ.get(profile.api_key_env or "XAI_API_KEY", "")
    return GrokProvider(
        name=profile.name,
...
def real_smoke(profile: ProviderProfile) -> dict:
    ...
    env = profile.api_key_env or "XAI_API_KEY"
