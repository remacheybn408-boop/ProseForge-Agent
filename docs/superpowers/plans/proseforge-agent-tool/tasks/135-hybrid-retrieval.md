# Task 135: Hybrid Retrieval / 混合检索

## Goal

把现有关键词搜索和向量语义搜索合并，提升 memory 和稿件检索质量。
...
```bash id="95aqsm"
pf-agent rag search "主角第一次发病" --slug demo_novel
```

## Files

- Create or modify implementation files under `src/proseforge_agent/retrieval/` as needed for this card.
- Add focused tests in `tests/retrieval/test_hybrid_retrieval.py`.
- Add fixtures under `tests/retrieval/fixtures/hybrid-retrieval/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/retrieval/test_hybrid_retrieval.py::test_hybrid_retrieval_contract`**

```python
def test_hybrid_retrieval_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 135 production code is not implemented yet.
    raise AssertionError("Task 135 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/retrieval/test_hybrid_retrieval.py::test_hybrid_retrieval_contract -q
```

Expected: FAIL because Task 135 production behavior is missing.
...
Expected: PASS with the new Task 135 behavior covered.
...
```powershell
git add src/proseforge_agent/retrieval/ tests/retrieval/test_hybrid_retrieval.py
...
同一个 query 同时返回关键词命中和语义相似结果，并给出 score。
...
```powershell
python -m pytest tests/retrieval/test_hybrid_retrieval.py -q
python -m pytest -q
```
...
Commit only Task 135 implementation files, tests, and required fixtures:
