# Task 94: Relationship Graph / 人物关系图

## Goal

建立人物、势力、组织之间的关系图，为写作 evidence 和分析提供结构化上下文。
...
```bash
pf-agent relation add --slug demo_novel
pf-agent relation list --slug demo_novel
pf-agent relation graph --slug demo_novel
```
...
- Add focused tests in `tests/novel/test_relationship_graph.py`.
- Add fixtures under `tests/novel/fixtures/relationship-graph/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/novel/test_relationship_graph.py::test_relationship_graph_contract`**

```python
def test_relationship_graph_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 94 production code is not implemented yet.
    raise AssertionError("Task 94 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/novel/test_relationship_graph.py::test_relationship_graph_contract -q
```

Expected: FAIL because Task 94 production behavior is missing.
...
Expected: PASS with the new Task 94 behavior covered.
...
```powershell
git add src/proseforge_agent/novel/ tests/novel/test_relationship_graph.py
git commit -m "feat: add relationship graph"
```
...
能导出人物关系图数据，并被 evidence pack 注入。
...
```powershell
python -m pytest tests/novel/test_relationship_graph.py -q
python -m pytest -q
```
...
Commit only Task 94 implementation files, tests, and required fixtures:
