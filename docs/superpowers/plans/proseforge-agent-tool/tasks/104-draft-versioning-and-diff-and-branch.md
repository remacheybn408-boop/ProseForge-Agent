# Task 104: Draft Versioning / Diff / Branch / 稿件版本管理

## Goal

支持章节版本、diff、分支、回滚，避免 Agent 改坏稿子无法恢复。
...
```bash
pf-agent draft version list --chapter ch_001
pf-agent draft diff draft_v1 draft_v2
pf-agent draft rollback --chapter ch_001 --to draft_v1
pf-agent draft branch --chapter ch_001 --name alt-ending
```

要求：

* 每次 rewrite 生成新 version
* 保留 checksum
* 保留 provider/prompt metadata
* 回滚需要 approval

## Files

- Create or modify implementation files under `src/proseforge_agent/novel/` as needed for this card.
- Add focused tests in `tests/novel/test_draft_versioning_and_diff_and_branch.py`.
- Add fixtures under `tests/novel/fixtures/draft-versioning-and-diff-and-branch/` only when deterministic sample data is needed.
...
- [ ] **Step 1: Write failing unit test `tests/novel/test_draft_versioning_and_diff_and_branch.py::test_draft_versioning_and_diff_and_branch_contract`**

```python
def test_draft_versioning_and_diff_and_branch_contract():
    # Replace this scaffold with the concrete assertion from the card goal.
    # The first run must fail because Task 104 production code is not implemented yet.
    raise AssertionError("Task 104 contract not implemented")
```

- [ ] **Step 2: Run the targeted test and confirm failure**

```powershell
python -m pytest tests/novel/test_draft_versioning_and_diff_and_branch.py::test_draft_versioning_and_diff_and_branch_contract -q
```

Expected: FAIL because Task 104 production behavior is missing.
...
Expected: PASS with the new Task 104 behavior covered.
...
```powershell
git add src/proseforge_agent/novel/ tests/novel/test_draft_versioning_and_diff_and_branch.py
git commit -m "feat: add draft versioning and diff and branch"
```
...
```powershell
python -m pytest tests/novel/test_draft_versioning_and_diff_and_branch.py -q
python -m pytest -q
```
...
Commit only Task 104 implementation files, tests, and required fixtures:
